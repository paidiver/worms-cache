...
class TaxonViewSetTests(APITestCase):
    ...
    def ajax_by_name_part_url(self, name_part: str) -> str:
        ...
        """
        return reverse("taxa-ajax-by-name-part", kwargs={"name_part": name_part})

    def ids_with_descendants_url(self) -> str:
        """Return the URL for the ids_with_descendants endpoint.

        Returns:
            str: The URL for the ids_with_descendants endpoint.
        """
    ...
    def test_list_taxa_filters_by_aphia_ids(self):
        ...
        resp = self.client.get(
            self.list_url(), [("aphia_ids[]", self.leaf.aphia_id), ("aphia_ids[]", self.root.aphia_id)]
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_ids_with_descendants_returns_taxon_and_all_descendants(self):
        ...
        resp = self.client.get(self.ids_with_descendants_url(), [("aphia_ids[]", self.root.aphia_id)])
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

        returned_ids = resp.data
        self.assertIn(self.root.aphia_id, returned_ids)
        self.assertIn(self.phylum.aphia_id, returned_ids)
        self.assertIn(self.leaf.aphia_id, returned_ids)
        self.assertIn(self.invalid.aphia_id, returned_ids)
