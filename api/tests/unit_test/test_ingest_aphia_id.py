...
class IngestAphiaIdTests(TestCase):
    ...
    @patch("api.services.ingest_aphia_id.WoRMSClient")
    def test_classification_deduplication_hits_processed_branch(self, MockClient: MagicMock):
        """Tests that if two AphiaIDs share part of their classification, the second one benefits from the first one's.
        ...
        leaf_ids = {LEAF_ID, PHYLUM_ID}

        record_by_id = RECORD_BY_ID

        def record_side_effect(aid: int):
            return record_by_id.get(int(aid))
    ...
    def test_vernacular_deduplication_hits_processed_branch(self, MockClient: MagicMock):
        """Tests that if two AphiaIDs share of their classification, the vernaculars of the shared is ingested once.
