...
These are the mapped endpoints from the WoRMS REST API:

* /AphiaRecordByAphiaID/{ID} -> /api/taxa/{aphia_id}/
* /AphiaChildrenByAphiaID/{ID} -> /api/taxa/{aphia_id}/?include_descendants=true
* /AphiaClassificationByAphiaID/{ID} -> /api/taxa/classification/{aphia_id}/
* /AjaxAphiaRecordsByNamePart/{NamePart} -> /api/taxa/ajax_by_name_part/{NamePart}/
* /AphiaRecordsByMatchNames -> /api/taxa/match_names/
...
| Service      | Purpose                                   |
| ------------ | ----------------------------------------- |
| `worms-db`   | PostgreSQL 16                             |
| `worms-cache`| Django REST API                           |
...
```bash
docker compose -f docker/docker-compose.yml exec worms-cache python manage.py makemigrations
```
...
```bash
docker compose -f docker/docker-compose.yml exec worms-cache python manage.py makemigrations --name migration_description
```
...
```bash
docker compose -f docker/docker-compose.yml run --rm worms-cache tox -e format
```

Run lint checks:

```bash
docker compose -f docker/docker-compose.yml run --rm worms-cache tox -e lint
```

Run the test suite with coverage:

```bash
docker compose -f docker/docker-compose.yml run --rm worms-cache tox -e py313
```
